...
//                Thread.currentThread().getContextClassLoader(); 
			ClassLoader classLoader = ClassPathXMLApplicationContext.class.getClassLoader();
		...
		try {
			...
			for (PropertyDescriptor proderdesc : ps) {
				...
				if (setter != null && setter.isAnnotationPresent(MyAnnotation.class)) {
					...
					MyAnnotation resource = setter.getAnnotation(MyAnnotation.class);
					String name = "";
					Object value = null;
	...
	public void fieldAnnotation(Object bean) {
		try {
			// 获取其全部的字段描述
//			Field[] fields = bean.getClass().getFields();//（java8不适用了？）
			Field[] fields = bean.getClass().getDeclaredFields();
			for (Field f : fields) {
				if (f != null && f.isAnnotationPresent(MyAnnotation.class)) {
					MyAnnotation resource = f.getAnnotation(MyAnnotation.class);
					String name = "";
					Object value = null;
					if (resource.name() != null && !"".equals(resource.name())) {
						name = resource.name();
	...
	public static void main(String[] args) {
		...
		MyAnnServiceImpl myAnnService = (MyAnnServiceImpl) path.getBean("myAnnService");
		myAnnService.show();
	}
}
